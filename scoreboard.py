...
class ScoreBoard:
    ...
    def prep_score(self):
        ...
        # prep left:
        left_score_str = f"$ {self.stats.score['left']}"
        self.left_score_image = self.font.render(
                left_score_str, True, self.text_color,
        ...
        # prep right
        right_score_str = f"$ {self.stats.score['right']}"
        self.right_score_image = self.font.render(
                right_score_str, True, self.text_color,
                self.settings.colors['bg_color'])
    ...
    def prep_lives(self):
        ...
        # prep left
        left_lives_str = '|'  * self.stats.player_lives['left']
        self.left_lives_image = self.font.render(
                left_lives_str, True, self.text_color,
                self.settings.colors['bg_color'])
        self.left_lives_rect = self.left_lives_image.get_rect()
        self.left_lives_rect.left = self.screen_rect.left + 20
        self.left_lives_rect.top = self.left_score_rect.bottom
        # Prep right
        right_lives_str = '|' * self.stats.player_lives['right']
        self.right_lives_image = self.font.render(
                right_lives_str, True, self.text_color,
                self.settings.colors['bg_color'])
        self.right_lives_rect = self.right_lives_image.get_rect()
        self.right_lives_rect.right = self.screen_rect.right - 20
    ...
    def display_score_and_lives(self):
        ...
        self.screen.blit(self.right_lives_image, self.right_lives_rect)
